...
# Generate index (merge with existing index to preserve older versions)
...
if [ -f "$CHARTS_DIR/index.yaml" ]; then
  ...
  helm repo index $CHARTS_DIR --url https://ninjatec.github.io/helmchecker --merge $CHARTS_DIR/index.yaml
else
...
if [ -f "$CHARTS_DIR/index.yaml" ]; then
  grep "version:" $CHARTS_DIR/index.yaml | grep -v "^apiVersion:" | sort -u
fi
echo ""
echo "Next steps:"
